...
By default, Timewarrior disables color output when its output is not a terminal (for example, when piped or run under 'watch').
One can force color output using the ':color' hint:
+
[source,shell]
----
timew :color day
----
+
This is particularly useful with 'watch' for live updates:
+
[source,shell]
----
watch -c timew :color day
----
+
where '-c' preserves ANSI color codes.
